...
double calculate_integral(double interval_start, double interval_stop, double (*fun)(double), double interval_width, long number_of_intervals){
    if(interval_stop - interval_start < interval_width)
        return fun((interval_start + interval_stop)/ 2.0)*(interval_stop - interval_start);

    double sum = 0.0;
    for(long i = 0; i < number_of_intervals; i++){
...
int main(int argc, char** argv) {
    ...
    long total_intervals_count = (long)ceil((double)(range_stop - range_start)/interval_width);
    long intervals_per_process = total_intervals_count/num_of_processes;
